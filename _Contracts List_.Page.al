page 50005 "Contracts List"
{
    ...
    area(navigation)
    {
      ...
      group(ActionGroupCRM)
      {
        ...
        action(CRMGoToSalesOrderListInNAV)
        {
          ...
          trigger OnAction()var CRMSalesorder: Record "CRM Salesorder";
          begin
    ...
    area(processing)
    {
      group(Action12)
      {
        ...
        action(Release)
        {
          ...
          trigger OnAction()var ReleaseSalesDoc: Codeunit "Release Sales Document";
          begin
      ...
      group("F&unctions")
      {
        ...
        action("Pla&nning")
        {
          ...
          trigger OnAction()var SalesOrderPlanningForm: Page "Sales Order Planning";
          begin
        ...
        action("Order &Promising")
        {
          ...
          trigger OnAction()var OrderPromisingLine: Record "Order Promising Line" temporary;
          begin
        ...
        action("Send IC Sales Order Cnfmn.")
        {
          ...
          trigger OnAction()var ICInOutboxMgt: Codeunit ICInboxOutboxMgt;
      ...
      group(Action3)
      {
        ...
        action("Create &Warehouse Shipment")
        {
          ...
          trigger OnAction()var GetSourceDocOutbound: Codeunit "Get Source Doc. Outbound";
          begin
  ...
  trigger OnFindRecord(Which: Text): Boolean var NextRecNotFound: Boolean;
  begin
    if not Find(Which)then exit(false);
    if ShowHeader then exit(true);
    repeat NextRecNotFound:=Next <= 0;
      if ShowHeader then exit(true);
  ...
  trigger OnNextRecord(Steps: Integer): Integer var NewStepCount: Integer;
  begin
    repeat NewStepCount:=Next(Steps);
    until(NewStepCount = 0) or ShowHeader;
    exit(NewStepCount);
  end;
  trigger OnOpenPage()var SalesSetup: Record "Sales & Receivables Setup";
  ...
  var ApplicationAreaMgmtFacade: Codeunit "Application Area Mgmt. Facade";
  ...
  procedure ShowPreview()var SalesPostYesNo: Codeunit "Sales-Post (Yes/No)";
  begin
    SalesPostYesNo.Preview(Rec);
  end;
  local procedure SetControlVisibility()var ApprovalsMgmt: Codeunit "Approvals Mgmt.";
  WorkflowWebhookManagement: Codeunit "Workflow Webhook Management";
  begin
  ...
  local procedure PostDocument(PostingCodeunitID: Integer)var LinesInstructionMgt: Codeunit "Lines Instruction Mgt.";
  begin
  ...
  procedure SkipShowingLinesWithoutVAT()begin
    SkipLinesWithoutVAT:=true;
  end;
  local procedure ShowHeader(): Boolean var CashFlowManagement: Codeunit "Cash Flow Management";
  begin
